...
enum NoteComposer: PredicateComposing {
	...
	private func searchPredicate(_ string: String ) -> SearchFor {
		SearchFor(.attribute("text"), that: .containsCaseInsensitive(string))
	}
	...
	func requirements() -> PredicateComposer? {
		...
		case .singleTag(let tags):
			return PredicateComposer(predicates: [SearchFor(.attribute("tags"), that: .haveAllOf(tags))])
		case .tags(let tags, let searchType):
			switch searchType {
			case .or:
				return PredicateComposer(predicates: [
					SearchFor(.attribute("tags"), that: .haveAtLeastOneOf(tags))
				])
			case .and:
				return PredicateComposer(predicates: [
					SearchFor(.attribute("tags"), that: .haveAllOf(tags))
				])
			}
		...
		case .alternativeSearch( let strings):
			return PredicateComposer(predicates:
										strings.map({ SearchFor(.attribute("text"), that: .containsCaseInsensitive( $0 )) }),
										   combinedWith: .or)
		case .beginsWith( let string ):
			return PredicateComposer(predicates: [SearchFor(.attribute("text"), that: .beginsWithCaseInsensitive(string))])
		case .allCompleted:
			return PredicateComposer(predicates: [SearchFor(.attribute("isCompleted"), that: .isTrue)])
		case .allNotCompleted:
			return PredicateComposer(predicates: [SearchFor(.attribute("isCompleted"), that: .isFalse)])
		}
	}
}
...
final class PredicateComposerTests: XCTestCase {
	...
	func test_PredicateComposer_arrayMatchLastTwoObjects_twoResults() throws {
		let search1 = SearchComposer(searches: [
			SearchFor(.entity, that: .isInArray(Array(exampleObjects.notes[1...2])))
		])

		let request = Note.fetchRequest()
		request.predicate = Note.generatePredicate(from: [search1])
		request.sortDescriptors = [NSSortDescriptor(key: "added", ascending: true)]
